...
export default function NewPropertyPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("properties").insert({
      ...
      city: formData.city,
      rooms: formData.rooms ? Math.round(Number(formData.rooms)) : null,
      area: formData.area ? parseFloat(formData.area) : null,
      rent_amount: isBuilding ? 0 : (formData.rent_amount ? Math.round(Number(formData.rent_amount.replace(/\s/g, ""))) : 0),
      charges: isBuilding ? 0 : (Math.round(Number(formData.charges.replace(/\s/g, ""))) || 0),
      sale_price: isBuilding ? null : (formData.sale_price ? Math.round(Number(formData.sale_price.replace(/\s/g, ""))) : null),
      owner_id: formData.owner_id || null,
      parent_id: formData.parent_id || null,
      unit_label: formData.unit_label || null,
      floor: formData.floor ? Math.round(Number(formData.floor)) : null,
      status: "AVAILABLE",
      photos: photos,
    });
